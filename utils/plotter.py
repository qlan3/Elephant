...
class Plotter(object):
    ...
    def merge_index(self, config_idx, mode, processed, exp=None):
        ...
        if (self.EMA) and (not processed):
            ...
            for i in range(len(xs)):
                ...
                result_list[i] = result_list[i][:n].copy()
                result_list[i].loc[:, self.x_label] = np.array(new_x, dtype=np.float32)
                result_list[i].loc[:, self.y_label] = np.array(new_y, dtype=np.float32)
        elif processed is False:
            # Moving average
            if self.rolling_score_window > 0:
                for i in range(len(result_list)):
                    new_x, new_y = (
                        result_list[i][self.x_label].to_numpy(),
                        result_list[i][self.y_label].to_numpy(),
                    )
                    new_y = moving_average(new_y, self.rolling_score_window)
                    result_list[i].loc[:, self.x_label] = np.array(
                        new_x, dtype=np.float32
                    )
                    result_list[i].loc[:, self.y_label] = np.array(
                        new_y, dtype=np.float32
                    )
    ...
    def get_result(self, exp, config_idx, mode, get_process_result_dict=None):
        ...
        if self.merged or processed:
            # Merge results
            print(
                f"[{exp}]: Merge {mode} results: {config_idx}/{get_total_combination(exp)}"
            )
            ...
            if processed:
                print(
                    f"[{exp}]: Process {mode} results: {config_idx}/{get_total_combination(exp)}"
                )
                for i in range(len(result_list)):
                    new_result = get_process_result_dict(
                        result_list[i], config_idx, mode
                    )
    ...
    def plot_vanilla(self, data, image_path):
        ...
        for i in range(len(data)):
            ...
            sns.lineplot(
                ...
                err_kws={"alpha": 0.5},
                linewidth=1.0,
                label=data[i][0][self.hue_label][0],
            )
    ...
    def plot_expIndexModeList(self, expIndexModeList, image_name):
        ...
        for exp, config_idx, mode in expIndexModeList:
            ...
            for i in range(len(result_list)):
                result_list[i][self.hue_label] = result_list[i][self.hue_label].map(
                    lambda x: f"[{exp}] {mode} {x} {config_idx}"
                )
    ...
    def plot_results(self, mode, indexes="all"):
        ...
        for config_idx in indexes:
            print(
                f"[{self.exp}]: Plot {mode} results: {config_idx}/{self.total_combination}"
            )
            # Get result
            try:
    ...
    def csv_merged_results(self, mode, get_csv_result_dict, get_process_result_dict):
        ...
        for config_idx in range(1, self.total_combination + 1):
            print(
                f"[{self.exp}]: CSV {mode} results: {config_idx}/{self.total_combination}"
            )
            result_list = self.get_result(
                self.exp, config_idx, mode, get_process_result_dict
            )
            if result_list is None:
                continue
    ...
    def csv_unmerged_results(self, mode, get_process_result_dict):
        ...
        for config_idx in range(1, self.runs * self.total_combination + 1):
            print(
                f"[{self.exp}]: CSV {mode} results: {config_idx}/{self.runs * self.total_combination}"
            )
    ...
    def compare_parameter(
        ...
        assert os.path.exists(results_file), (
            f"{results_file} does not exist. Please generate it first with csv_unmerged_results."
        )
        ...
        if kde:  # Plot all kdes in one figure
            ...
            fig, axs = plt.subplots(
                ...
                sharex=True,
                sharey=True,
                figsize=(7, 3 * len(param_values)),
            )
            if len(param_values) == 1:
                axs = [axs]
            for i, param_v in enumerate(param_values):
                sns.histplot(
                    data=results[results[param_name_short] == param_v],
                    x=perf,
                    hue=param_name_short,
                    kde=False,
                    stat=stat,
                    palette="bright",
                    ax=axs[i],
    ...
    def get_topn_result(
        ...
        # Select results based on the constraints and param
        def min_max(group):
            l = group.sort_values(by=perf, ascending=ascending)[:topn][perf].to_numpy(  # noqa: E741
                na_value=0
            )
    ...
    def get_top1_result(
        ...
        # Select results based on the constraints and param
        def top1(group):
            l = group.sort_values(by=perf, ascending=ascending)[:1]  # noqa: E741
            mean_value = l[perf].to_numpy(na_value=0)[0]
            errorbar_value = l[errorbar].to_numpy(na_value=0)[0]
            return pd.DataFrame(
                {"bmean+/-ci": [f"{mean_value:.{nd}f}+/-{errorbar_value:.{nd}f}"]}
            )

        # Get the intersection of two key list
        group_keys = list(set(group_keys).intersection(results.columns.tolist()))
        grouped_results = (
            results.groupby(by=group_keys).apply(top1).reset_index(level=-1, drop=True)
        )
        print("Performance measurement:", perf)
        print(f"Top {topn} results:")
        print(grouped_results)
...
def one_sided_ema(
    xolds, yolds, low=None, high=None, n=512, decay_steps=1.0, low_counts_threshold=0.0
):
...
def symmetric_ema(
    ...
    xs, ys1, count_ys1 = one_sided_ema(
        xolds, yolds, low, high, n, decay_steps, low_counts_threshold
    )
    _, ys2, count_ys2 = one_sided_ema(
        -xolds[::-1], yolds[::-1], -high, -low, n, decay_steps, low_counts_threshold
    )
    ys2 = ys2[::-1]
...
def export_legend(ax, path="./figures/legend.png"):
    fig2 = plt.figure()
