...
class Submitter(object):
    def __init__(self, general_cfg, sbatch_cfg):
        general_cfg.setdefault("jobs-per-task", 1)
        ...
        self.jobs_per_task = general_cfg["jobs-per-task"]
    ...
    def submit_multiple_jobs(self, num_jobs):
        ...
        with open(
            f"job_idx_{self.sbatch_cfg['job-name']}_{self.job_list[0]}.txt", "w"
        ) as f:
            f.write(job_indexes)
        ...
        bash_script = (
            f"sbatch --array={self.job_list[0]} {slurm_input} {self.script_path}"
        )
        ...
        while True:
            ...
            for line in lines:
                if self.user in line:
                    num_current_jobs += 1
            print(f"Number of current jobs: {num_current_jobs}")
            ...
            if (num_current_jobs < self.cluster_capacity) and (len(self.job_list) > 0):
                num_jobs = min(self.jobs_per_task, len(self.job_list))
                if num_jobs > 0:
    ...
    def single_submit(self):
        while True:
            ...
            for line in lines:
                if self.user in line:
                    num_current_jobs += 1
            print("Number of current jobs:", num_current_jobs)
            # If cluster capacity is not reached and job list is not empty, submit new jobs
            if (num_current_jobs < self.cluster_capacity) and (len(self.job_list) > 0):
                num_jobs = min(
                    self.cluster_capacity - num_current_jobs, len(self.job_list)
                )
                if num_jobs > 0:
