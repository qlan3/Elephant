...
from utils.helper import make_dir  # noqa: E402
...
class Sweeper(object):
    ...
    def get_num_combinations_of_dict(self, config_dict):
        ...
        """
        assert isinstance(config_dict, dict), "Config file must be a dict!"
    ...
    def get_num_combinations_of_list(self, config_list):
        ...
        assert isinstance(config_list, list), (
            "Elements in a config dict must be a list!"
        )
        num_combinations_of_list = 0
        for value in config_list:
            if isinstance(value, dict):
                if "num_combinations" not in value.keys():
    ...
    def generate_config_for_idx(self, idx):
        ...
        Generate a config dict for the index.
            If index < 0, set it to index+num_combinations+1.
            If index > 0, set it to index % num_combinations.
            Note that the first index is 1, not zero.
        """
        if idx < 0:
            idx = idx + self.config_dicts["num_combinations"] + 1
        assert idx > 0, "Index must >= -num_combinations."
        # Get config dict given the index
        cfg = self.get_dict_value(
            self.config_dicts, (idx - 1) % self.config_dicts["num_combinations"]
        )
        # Set config index
        cfg["config_idx"] = idx
    ...
    for i in range(runs * sweeper.config_dicts["num_combinations"]):
        ...
        try:
            with open(log_file, "r") as f:
                ...
                try:
                    t = float(last_line.split(" ")[-2])  # noqa: F841
                except:  # noqa: E722
                    print(i + 1, end=", ")
                    continue
        except:  # noqa: E722
...
if __name__ == "__main__":
    for agent_config in os.listdir("./configs/"):
        ...
        print(
            f"Number of total combinations in {agent_config}:",
            sweeper.config_dicts["num_combinations"],
        )
