...
from agents.RLTask import RLTask, TrainState
from networks import MLPGaussianActor, MLPVCritic
from utils.env import ppo_make_env
...
class PPO(RLTask):
    ...
    def __init__(self, cfg):
        ...
        self.train_iter = math.ceil(
            self.train_steps // self.cfg["agent"]["collect_steps"]
        )
        ...
        self.env = {
            "Train": ppo_make_env(
                self.task, gamma=self.discount, **self.cfg["task"]["kwargs"]
            ),
            "Test": ppo_make_env(
                self.task,
                gamma=self.discount,
                deque_size=cfg["test_episodes"],
        ...
        self.replay = FiniteReplay(
            self.cfg["agent"]["collect_steps"],
            keys=["obs", "action", "reward", "mask", "v", "log_pi"],
        )
    ...
    def createNN(self):
        ...
        if (
            "actor_model_cfg" in self.cfg["agent"].keys()
            and "critic_model_cfg" in self.cfg["agent"].keys()
        ):
            actor_model_cfg = self.cfg["agent"]["actor_model_cfg"]
            critic_model_cfg = self.cfg["agent"]["critic_model_cfg"]
        else:
            actor_model_cfg = self.cfg["agent"]["model_cfg"]
            critic_model_cfg = self.cfg["agent"]["model_cfg"]
        self.actor_net = MLPGaussianActor(
            action_size=self.action_size, **actor_model_cfg
        )
        self.critic_net = MLPVCritic(**critic_model_cfg)
        actor_optim = self.set_optim(
            self.cfg["optim"]["name"],
            self.cfg["optim"]["kwargs"],
        ...
        self.critic_state = TrainState.create(
            apply_fn=self.critic_net.apply, params=critic_params, tx=critic_optim
        )
    ...
    def run_steps(self, mode="Train"):
        ...
        for i in range(self.train_iter):
            for step in range(
                i * self.cfg["agent"]["collect_steps"],
                (i + 1) * self.cfg["agent"]["collect_steps"],
            ):
                # Get an action
                pred = self.get_action(step, obs[None,], mode)
                action, v, log_pi = pred["action"], pred["v"], pred["log_pi"]
                # Take a env step
                next_obs, reward, terminated, truncated, info = self.env[mode].step(
                    action
                )
                # Save experience
            ...
            # Update the agent
            self.actor_state, self.critic_state, self.seed = self.update(
                self.actor_state, self.critic_state, trajectory, obs[None,], self.seed
            )
            # Reset storage
            self.replay.clear()
    ...
    def get_action(self, step, obs, mode="Train"):
        if mode == "Train":
            action, v, log_pi, self.seed = self.random_action(
                self.actor_state, self.critic_state, obs, self.seed
            )
        else:  # mode == 'Test'
            action, v, log_pi, self.seed = self.optimal_action(
                self.actor_state, self.critic_state, obs, self.seed
            )
    ...
    def compute_advantage(self, critic_state, trajectory, last_obs):
        ...
        def _calculate_gae(gae_and_next_value, transition):
            ...
            delta = (
                transition["reward"] + transition["mask"] * next_value - transition["v"]
            )
    ...
    def update_epoch(self, carry, _):
        ...
        shuffled_trajectory = jax.tree_util.tree_map(
            lambda x: jax.random.permutation(update_seed, x, axis=0), trajectory
        )
        ...
        (actor_state, critic_state), _ = jax.lax.scan(
            f=self.update_batch, init=(actor_state, critic_state), xs=batches
        )
        carry = (actor_state, critic_state, trajectory, seed)
        return carry, None
    ...
    def update_batch(self, carry, batch):
        ...
        def compute_loss(params):
            ...
            # Compute actor loss
            action_mean, action_std = actor_state.apply_fn(
                params["actor"], batch["obs"]
            )
            pi = distrax.Normal(loc=action_mean, scale=action_std)
            ...
            total_loss = (
                actor_loss
                + self.cfg["agent"]["vf_coef"] * critic_loss
                - self.cfg["agent"]["ent_coef"] * entropy_loss
            )
            return total_loss

        # Update train_state and critic_state
        params = {"actor": actor_state.params, "critic": critic_state.params}
